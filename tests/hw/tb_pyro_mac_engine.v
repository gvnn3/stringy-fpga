...
module tb_pyro_mac_engine;
    ...
    // Exact DIG_CYCLES expectations under THIS testbench's feed cadence
    // (credit-handshake pulses, one byte per observed ready).  The
    // values pin the R45a latch: first-byte-accept to evt pulse.  Any
    // engine pipeline change that shifts them must be deliberate.
    localparam [31:0] EXP_DIGC_A = 32'd229; // frame A, 74 B digested
    localparam [31:0] EXP_DIGC_E = 32'd178; // frame E, 60 B ARP skip
    ...
    initial begin
        ...
        // (a-cnt) DIG_CYCLES/DIG_BYTES latched by frame (a): the R45a
        // per-frame pair, exact.  DIG_BYTES must equal the frame
        // length; DIG_CYCLES is pinned under this TB's feed cadence.
        $display("test a-cnt: DIG_CYCLES/DIG_BYTES latch");
        csr_r(A_DIG_BYTES, v);
        if (v !== 32'd74) begin
            $display("FAIL: DIG_BYTES %0d != 74", v);
            errors = errors + 1;
        end
        csr_r(A_DIG_CYCLES, v);
        $display("  DIG_CYCLES(a) = %0d", v);
        if (v !== EXP_DIGC_A) begin
            $display("FAIL: DIG_CYCLES %0d != expected %0d",
                     v, EXP_DIGC_A);
            errors = errors + 1;
        end
        ...
        // (e-cnt) skips latch the timing pair too (all three outcomes
        // latch; the reader knows the outcome from the reply flags).
        $display("test e-cnt: skip latches DIG_CYCLES/DIG_BYTES");
        csr_r(A_DIG_BYTES, v);
        if (v !== 32'd60) begin
            $display("FAIL: DIG_BYTES %0d != 60 after skip", v);
            errors = errors + 1;
        end
        csr_r(A_DIG_CYCLES, v);
        $display("  DIG_CYCLES(e) = %0d", v);
        if (v !== EXP_DIGC_E) begin
            $display("FAIL: DIG_CYCLES %0d != expected %0d",
                     v, EXP_DIGC_E);
            errors = errors + 1;
        end
