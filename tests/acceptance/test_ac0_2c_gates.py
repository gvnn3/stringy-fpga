...
def _assert_bytes_result_fidelity(pattern, subject):
    """All 8 APIs byte-identical to stock re AND element/return types are exactly
    ...
    for name in ("search", "match", "fullmatch"):
        ...
        assert isinstance(act, stdre.Match), f"{name} not genuine re.Match {tag}"
        ...
        d = {act.group(0): "v"}
        assert d[exp.group(0)] == "v", f"{name} group0 not dict-key-usable {tag}"
    ...
    assert act_sub == exp_sub and type(act_sub) is type(exp_sub), f"sub type {tag}"
    exp_sn = stdre.subn(pattern, repl, subject)
    act_sn = pre.subn(pattern, repl, subject)
    assert act_sn == exp_sn and type(act_sn[0]) is type(exp_sn[0]), f"subn {tag}"
...
@pytest.mark.parametrize("method", ["search", "match", "fullmatch", "findall", "finditer"])
...
SURR_SUBJECTS = [
    ("lone_high", "\ud800abcXYZ"),
    ("lone_low", "abc\udfffXYZ"),
    ("surrogateescape", b"\xff\xfe log line abc".decode("utf-8", "surrogateescape")),
...
@pytest.mark.parametrize("subj_label,subject", SURR_SUBJECTS, ids=[s[0] for s in SURR_SUBJECTS])
...
def test_surrogate_subject_short(pattern, subj_label, subject, mode):
    ...
    byte-identical to stock re and NO exception raised, on both routing modes."""
    _apply_mode(mode)
    fae0 = _fallback_after_error(pre)
    oracle.assert_equivalent(pre, pattern, subject, 0, label=f"surr:{subj_label}/{mode}")
    fae1 = _fallback_after_error(pre)
    if fae0 is not None and fae1 is not None:
        assert fae1 == fae0, "surrogate gate must not count as fallback_after_error (R14a)"


@pytest.mark.parametrize("mode", ["default", "force_model"])
def test_surrogate_subject_large_routes_to_fallback(mode):
    """R14a/R51a(d)/R29: a >= S_min surrogate subject (which would otherwise reach
    ...
    assert isinstance(m_pyro, stdre.Match), "surrogate gate must route to genuine fallback"
    assert oracle.canon_match(m_pyro) == oracle.canon_match(m_std)
    fae1 = _fallback_after_error(pre)
    if fae0 is not None and fae1 is not None:
        assert fae1 == fae0, "surrogate gate must not count as fallback_after_error (R14a)"
...
def test_surrogate_pattern(mode):
    """R14a/R8: a pattern containing an unpaired surrogate is non-UTF-8-encodable
...
def test_surrogate_pattern_classified_fallback():
    ...
    assert info["eligible"] is False, f"surrogate pattern must be fallback-only: {info}"
