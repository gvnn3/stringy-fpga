...
class TestStatsShape:
    def test_stats_reports_exactly_the_r66_counter_set(self):
        ...
        s = pre.stats()
        assert set(
    s.keys()) == ALL_COUNTERS, ( f"missing={
        ALL_COUNTERS - set(s)}, unexpected={
            set(s) - ALL_COUNTERS}")
        for k, v in s.items():
            assert type(v) is int, f"stats[{
    k!r}]={
        v!r} is {
            type(v).__name__}, not int"
    ...
    def test_stats_observation_is_non_perturbing(self):
        ...
        pre.search(
    "acc34observe",
     "z acc34observe z")  # make counters non-trivial
        s0 = pre.stats()
        for _ in range(25):
            assert pre.stats() == s0, "stats() observation moved a counter"

    def test_monotonic_counters_never_decrease_across_lifecycle(
        self, lifecycle_run):
        ...
        assert len(
            snaps) >= 8, f"too few snapshots: {[s['label'] for s in snaps]}"
    ...
    def test_gauge_circuits_synthesizing_rises_and_falls(self, lifecycle_run):
        ...
        assert lifecycle_run["reached"]["a"] is True, (
            "synthesis A never completed")
        peak = max(s["stats"]["circuits_synthesizing"]
        ...
        assert _snap(lifecycle_run, "synth_a_done")[
                     "circuits_synthesizing"] == 0
        assert _snap(lifecycle_run, "final_shutdown")[
                     "circuits_synthesizing"] == 0
...
class TestDispatchAttribution:
    ...
    def test_model_regime_call_moves_model_and_nothing_else(
        self, lifecycle_run):
    ...
    def test_hardware_counter_is_an_honest_zero(self, lifecycle_run):
        ...
        assert final["model"] >= 3, (
            f"model dispatches did not register: {final}")
        for s in lifecycle_run["snaps"]:
            assert s["stats"]["hardware"] == 0, ( f"hardware moved to {
    s['stats']['hardware']} at {
        s['label']!r} " "with no device present")
    ...
    def test_promotion_eviction_and_pr_loads_attribution(self, lifecycle_run):
        ...
        assert d["circuits_resident"] == 0, (
            "single-tenant swap changed the gauge")

    def test_lifecycle_results_identical_to_stock_throughout(
        self, lifecycle_run):
        ...
        for rec in lifecycle_run["results"]:
            assert rec["pyro"] is not None, f"{
    rec['label']}: pyro returned None"
...
class TestSynthFailureInjection:
    def test_launch_call_increments_synth_launched_and_synth_failed(
        self, synth_failure_runs):
    ...
    def test_pattern_becomes_permanently_fallback_only(
        self, synth_failure_runs):
        ...
        final = c[-1]["stats"]
        assert (final["fallback_after_error"] == 0
                and final["device_errors"] == 0)
        assert final["synth_launched"] == 1, "R65: synthesis was retried"
        assert first["circuit_status"] == "fallback_only"

    def test_results_byte_identical_throughout_failure_injection(
        self, synth_failure_runs):
    ...
    def test_device_error_full_counter_vector_and_results(self, kind):
        ...
        try:
            ...
            for i in range(n):
                ...
                assert got == expected, (
                    f"injected {kind} error altered result #{i}")
            _assert_only_moved(
                s0, pre.stats(),
