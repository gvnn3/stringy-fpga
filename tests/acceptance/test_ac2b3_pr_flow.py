"""AC-2b-3: PR-bitstream flow gating + manifest-layer honesty + JTAG load
surface.
...
(R85/R86.5) after which the loaded circuit answers a MATCH_REQUEST
byte-identically.
...
Testable WITHOUT hardware (this file):
  * R72c/R82c honesty: while `pr_flow_present` is false, NO produced manifest
  may
  ...
  * R47b/R82c `pr_verified: bool` (v2.2.2): the INDEPENDENT observable seam for
    pr_verify success — default False, JSON back-compat (absent key → False),
    True
...
# ==========================================================================
# On-device PASS is gated: SKIP until pr_flow_present ∧ device_usable
# (R83/R82d).
# ==========================================================================
# AC-2b-3 (R71/R82/R83/R85)
def test_on_device_pr_bitstream_roundtrip_is_gated():
    ...
    if not (pr_ok and dev_ok):
        reasons = [
    r for ok, r in (
        (pr_ok, pr_reason), (dev_ok, dev_reason)) if not ok]
        pytest.skip("; ".join(reasons))
    raise AssertionError(
        "pr_flow_present ∧ device_usable unexpectedly true — implement the "
        "in-context pr_bitstream build + pr_verify + JTAG load + MATCH "
        "round-trip "
...
# The probe consults the three R68 knobs (PYRO_PR_STATIC_DCP /
# PYRO_PR_REFERENCE_DCP /
# PYRO_PR_EVIDENCE_MANIFEST) in fixed order; the first unmet condition is
# named in the
# canonical `pr_flow_present=false — <first unmet R83a condition>` reason
# (R83/R83a).
# It flips TRUE iff the evidence manifest parses (R47b-consistency loader) and
# attests a
...
_R83A_STATIC = ("pr_flow_present=false — static DCP not configured/unreadable "
                "(PYRO_PR_STATIC_DCP)")
_R83A_REFERENCE = (
    "pr_flow_present=false — reference DCP not configured/unreadable "
    "(PYRO_PR_REFERENCE_DCP)")
...
_R83A_NOT_VERIFIED = (
    "pr_flow_present=false — evidence manifest not pr_verified "
    "(no verified pr_bitstream attested)")


def _assert_no_stale_placeholder(reason):
    """The sweep must retire every pre-R83a placeholder (PYRO_PR_FLOW env var,
    the
    'not built/validated' literal, and the v2.2.4 'not verifiable host-side'
    conservative
    ...
    for marker in _STALE_MARKERS:
        assert marker not in reason, f"stale placeholder {
    marker!r} survived: {
        reason!r}"


def _fake_dcp(path):
    """A readable stand-in DCP (the probe only checks existence+readability at
    arms 1/2,
...
def _write_manifest_json(path, *, pr_verified, payload_kind):
    """Write a schema-correct pyro.synth.Manifest JSON sidecar via the public
    API, so
    the R83a arm-4 loader (Manifest.from_json) parses it exactly as the spec
    defines."""
...
def _set_substrate(monkeypatch, tmp_path):
    """Configure arms 1+2 (both substrate DCPs present+readable) so a test can
    drive
...
def test_r83a_arm1_static_unset(monkeypatch):  # AC-2b-3 (R83a.1)
    """R83a step 1: with all knobs unset, the first unmet condition is the
    static DCP."""
    for k in (
    "PYRO_PR_STATIC_DCP",
    "PYRO_PR_REFERENCE_DCP",
...
# AC-2b-3 (R83a.1)
def test_r83a_arm1_static_configured_but_missing(tmp_path, monkeypatch):
    """R83a step 1: a configured-but-unresolvable static DCP path is
    'unreadable' — same
...
def test_r83a_arm2_reference_unset(tmp_path, monkeypatch):  # AC-2b-3 (R83a.2)
    """R83a step 2 (fixed order): static present+readable, reference unset →
    reference."""
    monkeypatch.setenv(
    "PYRO_PR_STATIC_DCP",
    _fake_dcp(
...
def test_r83a_arm3_evidence_unset(tmp_path, monkeypatch):  # AC-2b-3 (R83a.3)
    """R83a step 3: both DCPs present, evidence-manifest knob unset → evidence
    config."""
...
def test_r83a_arm3_evidence_configured_but_missing(tmp_path, monkeypatch):
    # AC-2b-3 (R83a.3)
    """R83a step 3: a configured-but-nonexistent evidence path is 'unreadable'
    — same
...
def test_r83a_arm4_corrupt_json(tmp_path, monkeypatch):  # AC-2b-3 (R83a.4)
    """R83a step 4: a corrupt (non-JSON) evidence file fails
    Manifest.from_json → the
...
def test_r83a_arm4_inconsistent_manifest_rejected(tmp_path, monkeypatch):
    ...
    """R83a step 4 + R47b-consistency (v2.2.3): an internally-inconsistent
    manifest
    (pr_verified==True but payload_kind!='pr_bitstream') MUST be rejected by the
    R47b-consistency-enforcing loader, so the probe reports 'not parseable' —
    it is NOT
    silently accepted and MUST NOT flip true.  Built via raw JSON so it
    bypasses the
    (also-rejecting) constructor."""
    _set_substrate(monkeypatch, tmp_path)
    data = json.loads(psynth.Manifest(**_MANIFEST_BASE).to_json())
    data["pr_verified"] = True
    # inconsistent: verified claim, non-PR payload
    data["payload_kind"] = "ooc_metrics"
    ...
    present, reason = phase2_support.pr_flow_present()
    assert present is False, (
        "an inconsistent evidence manifest must NOT flip pr_flow_present true "
        "(R47b-"
        "consistency / R83a.4)")
    # Spec-correct disposition is arm 4 (rejected at from_json).  If a build
    # ever failed
    # to enforce R47b-consistency, from_json would accept it and the probe
    # would fall to
...
def test_r83a_arm5_not_verified(
    tmp_path,
    monkeypatch,
    pr_verified,
     payload_kind):
    # AC-2b-3 (R83a.5)
    """R83a step 5: a parseable, CONSISTENT manifest that does not attest a
    verified
    pr_bitstream (payload_kind!='pr_bitstream' / pr_verified!=true) → 'not
    pr_verified'.
    """
    _set_substrate(monkeypatch, tmp_path)
...
# AC-2b-3 (R83a full)
def test_r83a_full_true_flips_present(tmp_path, monkeypatch):
    """R83a: with both substrate DCPs present+readable AND an evidence
    manifest that
    parses and attests payload_kind=='pr_bitstream' with pr_verified==true,
    the probe
    flips TRUE with an empty reason.  This is the sole host-observable true
    polarity
    ...
    assert present is True, (
        f"all five R83a conditions satisfied but probe stayed false: "
        f"{reason!r}")
    assert reason == "", f"true polarity must carry an empty reason, got {
    reason!r}"


# ---- Opportunistic real-artifact check (non-hermetic; skips if absent) ----
# AC-2b-3 (R83a.4/.5)
def test_r83a_real_evidence_manifest_if_present(monkeypatch):
    """Opportunistic (non-hermetic): if the first real PR job has emitted an
    evidence
    manifest at /usr/local/cad/gn262/pyro/patterns/pattern_*_manifest.json,
    assert it
    satisfies R83a arms 4-5 (parses via the R47b-consistency loader AND
    attests a
    ...
    if not candidates:
        pytest.skip(
    "no real PR evidence manifest present yet "
    "(/usr/local/cad/gn262/pyro/patterns/pattern_*_manifest.json)")
    real = candidates[0]
    ...
    # And end-to-end: pointing the probe at it flips pr_flow_present true
    # (needs the
    # substrate DCP knobs too; use the same real artifacts as the operator
    # would).
    static = ("/usr/local/cad/gn262/pyro/open-nic-shell/build/"
              "au250_pyro_pr/pr/pyro_static_locked.dcp")
    reference = ("/usr/local/cad/gn262/pyro/open-nic-shell/build/"
                 "au250_pyro_pr/pr/pyro_static_id_stub_routed.dcp")
    ...
    present, reason = phase2_support.pr_flow_present()
    assert present is True, f"real R83a inputs present but probe false: {
    reason!r}"
...
# ==========================================================================
# AC-2b-3 (R72c/R82c)
def test_no_manifest_claims_pr_bitstream_while_pr_flow_absent():
    """LIVE: while pr_flow_present is false, no manifest anywhere in the session
    bitstream cache may claim payload_kind == "pr_bitstream" (R72c/R82c
    honesty).
    ...
    assert not offenders, (
        f"manifests falsely claim payload_kind=='pr_bitstream' without a "
        f"passing "
...
def test_pr_verified_defaults_false():  # AC-2b-3 (R47b)
    """R47b: pr_verified default is False (an unqualified manifest is not
    verified)."""
...
def test_pr_verified_json_roundtrip(value, payload_kind):  # AC-2b-3 (R47b)
    """R47b: pr_verified survives a to_json -> from_json round-trip exactly,
    like
    payload_kind."""
    m = psynth.Manifest(
    **_MANIFEST_BASE,
...
# AC-2b-3 (R47b back-compat)
def test_pr_verified_absent_key_is_false_backcompat():
    """R47b: from_json of a legacy manifest with NO pr_verified key defaults
    it to
    False (back-compat exactly like payload_kind).  Absent-key manifests are
    always
    mock_stub/ooc_metrics (pr_verified False), so they satisfy
    R47b-consistency."""
    data = json.loads(psynth.Manifest(**_MANIFEST_BASE).to_json())
...
# ---
# Enforcement is landing in parallel (coder); a manifest that is ACCEPTED
# where the
...
def _assert_rejects(build):
    """R47b-consistency: `build()` MUST raise ValueError or a pyro.synth
    manifest
    error.  Accept either; distinguish pending-impl (accepted) from a wrong
    type."""
    try:
        build()
    except ValueError:
        # accepted taxonomy (R47b-consistency)
        return
    except Exception as exc:  # noqa: BLE001
        if type(exc).__module__.startswith("pyro"):
            # a pyro.synth manifest error is fine
            return
        pytest.fail(
            f"R47b-consistency must raise ValueError / a pyro.synth manifest "
            f"error, "
            f"got {type(exc).__module__}.{type(exc).__name__}: {exc}")
    else:
        pytest.xfail(
    "R47b-consistency boundary rejection not yet landed (v2.2.3; "
    "coder implementing in parallel)")


@pytest.mark.parametrize("pv,pk", _INCONSISTENT,
                         ids=[f"{pv}+{pk}" for pv, pk in _INCONSISTENT])
# AC-2b-3 (R47b-consistency)
def test_manifest_ctor_rejects_inconsistent(pv, pk):
    """R47b-consistency (v2.2.3): the Manifest constructor MUST reject a
    manifest
...
@pytest.mark.parametrize("pv,pk", _INCONSISTENT,
                         ids=[f"{pv}+{pk}" for pv, pk in _INCONSISTENT])
# AC-2b-3 (R47b-consistency)
def test_manifest_from_json_rejects_inconsistent(pv, pk):
...
def test_manifest_accepts_consistent(pv, pk):  # AC-2b-3 (R47b-consistency)
    """The consistent combinations MUST still be accepted by ctor and
    from_json."""
...
# AC-2b-3 (R47b/R82c)
def test_mock_produced_manifest_carries_pr_verified_false():
    """R47b/R82c: a manifest produced by the MOCK toolchain (a mock_stub
    payload)
    MUST carry pr_verified == False — no pr_verify ran.  Produced
    hardware-free via
    ...
    "an error and warn",
     cache_dir=cache)
        ...
        for path, data in mans:
            assert data.get("payload_kind") in (
    "mock_stub", "ooc_metrics"), ( f"{path}: unexpected payload_kind {
        data.get('payload_kind')!r} on " "the mock path (R72a)")
            assert data.get("pr_verified", False) is False, (
                f"{path}: mock/ooc manifest must have pr_verified==False "
                f"(R47b/R82c)")


def _assert_pr_verified_consistent(path, data):
    """R82c consistency invariant: pr_verified == True IFF payload_kind ==
    'pr_bitstream'.  A manifest claiming pr_verified with a non-PR
    payload_kind (or
    ...
    if pv:
        assert pk == "pr_bitstream", (
            f"{path}: pr_verified==True but payload_kind=={pk!r} — a verified "
            f"claim "
...
def test_all_produced_manifests_are_pr_verified_consistent():  # AC-2b-3 (R82c)
    """LIVE: every manifest reachable in the session cache AND a fresh mock
    synthesis
    ...
    with tempfile.TemporaryDirectory(prefix="pyro_ac2b3_cons_") as cache:
        ...
        for path, data in phase2_support.find_manifests(cache):
            ...
            checked += 1
    assert checked >= 1, (
        "no manifests were available to check consistency against")
...
# ==========================================================================
def _make_config():
    """R86.6 JTAG/hw_server config built directly from the normative
    DeviceConfig
    field names.  Defaults already carry an hw_server locator that is not
    running on
    this host (R85); a frozen dataclass rejects unknown kwargs (no silent
    drop)."""
...
def test_load_partial_raises_pyro_load_error_on_mechanism_failure(tmp_path):
    # AC-2b-3 (R86.1/R86.5)
    """R86.5: a JTAG/hw_server mechanism failure MUST surface as
    PyroLoadError, not
    OSError/PermissionError or a transport internal (R86.1).  With no
    hw_server and
    ...
    # pragma: no cover - spec violation
        ...
        pytest.fail(
    "load_partial returned success with no hw_server and a bogus "
