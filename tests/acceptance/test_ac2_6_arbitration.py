...
single-tenant eviction policy (R64) and byte-identical results across
evict/reload
...
budget — so this is a FIRM, non-vacuous LIVE assertion.  The multi-pattern
policy
...
# Distinct HW-eligible literal patterns; more than the single-tenant budget so
# at
...
def _pr_partitions():
    """R42/R64 single-tenant budget from the model caps; default 1
    (single-tenant)
...
def test_on_device_arbitration_skips():
    ...
    if not (dev_ok and pr_ok):
        reasons = [
    r for ok, r in (
        (dev_ok, dev_reason), (pr_ok, pr_reason)) if not ok]
...
def test_model_single_tenant_eviction_and_byte_identical():
    """LIVE (model): loading several distinct patterns' circuits keeps the
    resident
    ...
    # Keystone: results are byte-identical across evict/reload regardless of
    # tier.
    ...
    if res["max_resident_gauge"] >= 1 and res["n_patterns"] > pr_partitions:
        assert res["evicted_delta"] >= 1, (
            f"{res['n_patterns']} patterns loaded into {pr_partitions} "
            f"partition(s) "
