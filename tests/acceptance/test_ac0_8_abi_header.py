...
assertion is a **historical Phase-0 checkpoint against the stub, not an
invariant
...
@pytest.mark.skipif(_lib is None,
...
@pytest.mark.skipif(not (CC and MAKE),
     reason="need make + cc to compile the ABI check")
def test_make_abi_check_reports_2_0_0():
    """R37/R38: the Makefile `abi-check` target compiles the frozen header
    against
...
def test_abi_1_0_0_is_phase0_historical_checkpoint():
    ...
    Phase-0 stub source is absent here (its checkpoint lives on
    `phase0-pyro`)."""
    ...
    assert not os.path.isfile(STUB_1_0_0), (
        "src/pyro_rt_stub.c (the 1.0.0 stub) should not exist on the "
        "integrated "
...
def test_header_declares_2_0_0_and_records_supersession():
    ...
    assert "1.0.0" in text, (
        "header should record the superseded 1.0.0 checkpoint (R37)")
