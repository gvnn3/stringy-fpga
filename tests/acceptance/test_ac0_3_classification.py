...
def _assert_eligible(pattern, flags=0):
    ...
    assert isinstance(
    info, dict), f"explain() must return dict, got {
        type(info)}"
    for key in ("eligible", "reason", "engine", "states"):
        assert key in info, f"explain() missing key {key!r}: {info}"
    assert info["eligible"] is True, f"expected eligible for {
    pattern!r}: {info}"
    assert info["engine"] in (
    "fpga", "model"), f"engine for eligible {
        pattern!r}: {info}"
...
def _assert_fallback(pattern, flags=0):
    ...
    for key in ("eligible", "reason", "engine", "states"):
        assert key in info, f"explain() missing key {key!r}: {info}"
    assert info["eligible"] is False, f"expected fallback-only for {
    pattern!r}: {info}"
    assert info["engine"] == "fallback", f"engine must be 'fallback' for {
    pattern!r}: {info}"
...
@pytest.mark.parametrize("entry", oracle.UNSUPPORTED,
                         ids=[e[0] for e in oracle.UNSUPPORTED])
...
def test_unsupported_results_correct_via_fallback(entry):
    """R8/R29: fallback-only §5.2 patterns still return byte-identical
    results."""
