"""AC-2b-2: live device probe pyro.device.probe_device — privilege-free path LIVE.

R86.4/R83/R84: probe_device(config) -> (usable: bool, reason: str) implements the
R83/R84 device-usability probe.  The privilege-free (False, reason) path is LIVE
on THIS host today: with no CAP_NET_RAW it MUST detect that BEFORE any privileged
AF_PACKET operation and return (False, <R83 canonical reason>) WITHOUT raising
PermissionError/OSError (R86.4).

The usable==true flip requires a flashed PYRO PR shell answering ID_REQUEST with a
SPEC16-valid ID_REPLY AND CAP_NET_RAW (R83 (i)+(ii)); on this host that path
records a SKIP naming the missing prerequisite (R71 SKIP discipline, never PASS).

Derived ONLY from the spec.  Device-probe assertions are pending until pyro.device
...
v2.2.2 (R86.4): the current-host reason is now the EXACT, non-elided two-condition
...
def _make_config():
    ...
    names (iface = F3 onic netdev, expected_spec16 = R81 0x0202).  DeviceConfig is a
...
# ==========================================================================
def test_probe_privilege_free_exact_canonical_reason():  # AC-2b-2 (R86.4 exact literal)
    """LIVE on this host (no CAP_NET_RAW): probe_device returns (False, reason)
    WITHOUT raising PermissionError/OSError, and `reason` is the EXACT, non-elided
    two-condition canonical literal R86.4 fixes for this condition set (both probe
    ...
    except (PermissionError, OSError) as exc:  # pragma: no cover - spec violation
        pytest.fail(f"probe_device leaked {type(exc).__name__} on the privilege-"
                    f"free path (R86.4 forbids requiring privilege for False)")
    assert usable is False, "no CAP_NET_RAW and no flashed shell => not usable (R83)"
...
def test_probe_never_raises_permission_error():  # AC-2b-2 (R86.4)
    ...
    (only a genuinely malformed reply may raise PyroFrameError, which needs a live
...
def test_device_usable_flip_is_skip_until_probe_confirms():  # AC-2b-2 (R71/R83)
    """The (True, reason) flip requires a flashed SPEC16-valid PR shell + CAP_NET_RAW
    (R83 (i)+(ii)).  Neither holds here, so this records a SKIP whose reason names
    ...
    raise AssertionError(
        "device_usable unexpectedly true — implement the on-device probe-confirmed "
...
# ==========================================================================
def test_phase2_support_reason_is_r83_canonical():  # AC-2b-2 (R83 supersedes v2.1.3)
    usable, reason = phase2_support.device_usable()
    ...
    # On this host (no CAP_NET_RAW) the exact canonical enumeration is both unmet.
