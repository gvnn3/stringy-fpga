...
LIVE clause (toolchain_present, R71): the cold->warm leg is real Vivado
synthesis
...
searches proceed byte-identically while synthesis is in flight, and the
synthesis
...
def test_model_async_prewarm_is_nonblocking_all_cases():
    ...
    service synthesizes; the pattern reaches a terminal tier and no error
    counter
    moves (R63/R58a).  Runs on the mock toolchain regardless of vivado
    presence."""
    ...
    res['prewarm_seconds']}s — synthesis must be async (R63)")
...
def test_live_real_synth_is_async_and_never_blocks_caller(vivado_corpus):
    """LIVE: real Vivado synthesis is async and never blocks/slows a caller —
    the
    ...
    assert w["prewarm_seconds"] < 3.0, (
        f"prewarm blocked {w['prewarm_seconds']}s while enqueuing real "
        f"synthesis "
    ...
    assert w["saw_synthesizing_or_cold"] is True, (
        "never observed the pattern cold/synthesizing — cannot prove calls "
        "were "
    ...
    assert w["search_all_eq_stock"] is True, (
        "a call served during synthesis was NOT byte-identical to stock re "
        "(R16)")
    assert w["final_eq_stock"] is True


def test_live_real_synth_genuinely_takes_minutes(vivado_corpus):
    """LIVE: the cold->warm leg is genuine (real Vivado OOC synth+P&R is
    minutes,
    not instant).  Soft lower bound as evidence of real synthesis work
    (R63/P1)."""
    w = _primary(vivado_corpus)[1]["worker"]
    assert w["reached_terminal"] is True, w["last"]
    assert w["synth_wall_seconds"] >= 20.0, (
        f"real Vivado synthesis completed in {w['synth_wall_seconds']}s — too "
        f"fast "
...
def test_live_cold_to_warm_persists_across_restart(vivado_corpus):
    ...
    sharing PYRO_CACHE_DIR loads the artifact warm in PR-load time, not
    synthesis
