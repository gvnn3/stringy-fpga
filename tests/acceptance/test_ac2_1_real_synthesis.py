...
LIVE clauses (require toolchain_present, R70/R71):
  * a HW-eligible pattern is synthesized by REAL Vivado OOC synth+P&R for the
    ...
    GENUINE post-route luts/ffs (>0) (R72c), met_timing True and fmax_mhz
    implied
    by met_timing at the 250 MHz proxy clock (R73), toolchain_version encoding
    the
    real Vivado 2025.2 (0x19020000, != mock 0x00000100) (R70a-pin/R75),
    fitting the
    R11 PR budget;
  * the artifact populates the bitstream cache and reloads WARM across a
  simulated
    process restart sharing PYRO_CACHE_DIR (R4/R63d).

SKIP clause (requires pr_flow_present, R71 — FALSE here):
  * production of a genuine loadable PR bitstream
  (payload_kind=="pr_bitstream").

NOTE (slow): each real Vivado run is minutes; the calibration corpus is
synthesized ONCE by the session-scoped `vivado_corpus` fixture and reused here
and
in AC-2-3/AC-2-4.  When toolchain_present is false (the default mock config),
the
...
def _primary(corpus):
    ...
    if pat is None:
        ...
        pytest.fail(
    f"no calibration pattern's vivado worker ran (all crashed): {errs}")
    return corpus[pat]
...
def test_real_ooc_manifest_is_honest_ooc_metrics(vivado_corpus):
    """LIVE: the real Vivado flow writes exactly one ooc_metrics manifest
    carrying
...
def test_real_ooc_met_timing_and_fmax_proxy(vivado_corpus):
    """LIVE: met_timing True at the 250 MHz proxy and fmax_mhz implied by
    met_timing
    (WNS>=0 => fmax = 1000/(4.000 - WNS) >= 250) (R73)."""
    man = _primary(vivado_corpus)["manifest"]
    assert man is not None, (
        "no ooc_metrics manifest — real synthesis did not succeed (R72c)")
    assert man["met_timing"] is True, (
        "a synthesized+cached ooc artifact must have met timing at 250 MHz "
        "(R73); "
    ...
    assert float(
    man["fmax_mhz"]) >= float(
        phase2_support.PROXY_CLOCK_MHZ), ( f"fmax_mhz {
            man['fmax_mhz']} < 250 MHz contradicts met_timing==True (R73)")


def test_real_toolchain_version_is_vivado_not_mock(vivado_corpus):
    """LIVE: toolchain_version encodes the pinned real Vivado 2025.2
    (0x19020000)
    ...
    if "shell_version" in man:
        assert (int(man["shell_version"])
                == phase2_support.SHELL_VERSION_MODEL), (
...
def test_real_ooc_manifest_fits_pr_budget(vivado_corpus):
    ...
    if caps is None:
        pytest.skip(
    "model caps unavailable (libpyro_rt.so not built); budget-fit "
    "is implied by a successful ooc synthesis (R12)")
    assert man["luts"] <= caps["pr_luts"], ( f"post-route luts {
    man['luts']} exceed PR budget {
        caps['pr_luts']} (R11)")
...
def test_real_artifact_reloads_warm_across_restart(vivado_corpus):
    """LIVE: a FRESH process sharing PYRO_CACHE_DIR finds the vivado artifact
    WARM
...
def test_no_ooc_manifest_claims_pr_bitstream(vivado_corpus):
    """R72/R72c honesty: with pr_flow_present false, NO manifest may claim to
    be a
    loadable device bitstream (payload_kind must never be 'pr_bitstream'
    here)."""
    for entry in vivado_corpus.values():
        for _p, man in phase2_support.find_manifests(entry["cache"]):
            assert man.get("payload_kind") != "pr_bitstream", (
                "no loadable PR bitstream can be produced without "
                "pr_flow_present "
...
def test_loadable_pr_bitstream_requires_pr_flow():
    ...
    (payload_kind=='pr_bitstream') requires pr_flow_present, FALSE here
    (R71/R72)."""
    present, reason = phase2_support.pr_flow_present()
    if not present:
        pytest.skip(reason)
    # Unreachable on this host; a real PR flow would assert a pr_bitstream
    # artifact.
    raise AssertionError(
    "pr_flow_present unexpectedly true — implement PR-bitstream "
