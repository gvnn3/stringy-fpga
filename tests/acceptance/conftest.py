...
# Env vars sampled at R35a points (import/install/uninstall/refresh_env). All
# are
...
_ENV_KEYS = ("PYRO_DISABLE", "PYRO_FORCE_MODEL", "PYRO_ENABLE_TEST_HOOKS",
             "PYRO_N_SYNTH",
             # R68/R70 (v2.1.0) toolchain-selection knobs. Sampled at R35a
             # points;
             ...
             # without leaking the vivado toolchain into Phase-0/1 mock-only
             # tests.
             "PYRO_TOOLCHAIN", "PYRO_VIVADO",
             # R68 device knobs (v2.2.2): the onic netdev name and hw_server
             # URL that
             # seed DeviceConfig defaults (R86.6), sampled at R35a points.
             # Saved/
             # cleared/restored per test so knob-sampling tests leave no env
             # residue.
             "PYRO_DEVICE_IFACE", "PYRO_HW_SERVER",
             # R68 PR-substrate knobs (v2.2.4): the locked-static and
             # reference-routed
             # DCP paths that populate
             # ToolchainConfig.static_dcp/reference_dcp (R82b/
             # R82c/R82d) and that the phase2_support.pr_flow_present probe
             # consults
             # (R83). No default; fail-loud (R68/R88). Saved/cleared/restored
             # per test
             # so the both-polarity pr_flow_present checks leave no env residue.
             "PYRO_PR_STATIC_DCP", "PYRO_PR_REFERENCE_DCP",
             # R68 PR evidence-manifest knob (v2.2.5): the host-observable
             # proxy for a
             # passing pr_verify (R82c) that flips pr_flow_present true
             # (R83a). No
             # default; fail-closed. Populates
             # ToolchainConfig.pr_evidence_manifest.
             # Saved/cleared/restored per test so the full-true polarity
             # leaves no residue.
             "PYRO_PR_EVIDENCE_MANIFEST",
             # Harness-only knob (Phase 1/2/3 workers +
             # phase3_support.poll_tier):
             # worker-side synthesis-poll deadline in seconds. Not sampled by
             # pyro
             # itself, but registered here so a test that writes it into
             # os.environ
...
# phase2_support.toolchain_present() PINS PYRO_TOOLCHAIN=vivado / PYRO_VIVADO
# into
# os.environ (a raw persistent write, needed so out-of-process synth workers
# select the
# real OOC adapter). It is called from the SESSION-scoped `vivado_corpus`
# fixture, which
# is set up BEFORE the function-scoped `pyro_isolation` fixture for the first
# requesting
# test — so pyro_isolation captures the ALREADY-pinned 'vivado' as its per-
# test baseline
# and "restores" to 'vivado', letting the pin outlive the suite and leak into
# tests/unit
# in a combined `pytest tests/acceptance tests/unit` run (mock-expecting unit
# tests then
# launch real Vivado synth). We snapshot the PRISTINE toolchain selection at
# conftest
# import (before any fixture can pin it) and force these two keys back to it
# in every
# pyro_isolation teardown, so the pin can never survive past the test that
# set it.
_PRISTINE_TOOLCHAIN = {
...
def pytest_configure(config):
    ...
    "markers",
     "perf: performance / routing-overhead tests (AC-0-6, R3a/R5)" )
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if not _DEFAULT_CACHE_PREEXISTED and os.path.exists(_DEFAULT_CACHE):
        raise AssertionError(
            f"cache residue leaked to {_DEFAULT_CACHE}; PYRO_CACHE_DIR "
            f"redirect "
    ...
    try:
        ...
        for k, v in saved.items():
            if v is None:
                ...
                os.environ[k] = v
        # Defeat the polluted-baseline ordering bug: force the toolchain-
        # selection keys
        # back to the PRISTINE session snapshot (not the possibly-vivado-
        # pinned per-test
        # baseline) so a session-fixture pin never outlives its test or leaks
        # to tests/unit.
