...
(R71), BOTH established false on this host (third-party live NIC that must not
be
perturbed; no OpenNIC PR floorplan / PR-bitstream flow).  Per the R71 SKIP
discipline it records a SKIP whose reason names the missing prerequisites —
NEVER
a PASS.
...
def test_on_device_harness_contract_over_1mib():
    ...
    # Unreachable on this host.  Model-side correctness is AC-1-3 (not a PASS
    # here).
    raise AssertionError(
        "device_usable ∧ pr_flow_present unexpectedly true — implement "
        "on-device "
