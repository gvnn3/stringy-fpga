...
def _emit_rtl_group(automata, circ_id, circ_flags, n_slots: int, n: int) -> str:
    ...
    add("// Auto-generated by pyro.hdl.generator — do not edit.")
    add(f"// PYRO pattern-SET recognizer (SNORT-PF SR7): {n_slots} independent")
    add(f"// automata sharing ONE harness; {n} byte(s)/cycle; pattern_id = slot.")
    add(f"// generator_version=0x{GENERATOR_VERSION:08X} "
        f"harness_version=0x{HARNESS_VERSION:08X} datapath_bytes={n}")
    add("//")
    add("// Accept arbitration: a consumed beat latches every (lane, slot) accept")
    add("// into `pend`; a priority encoder drains ONE result-ring entry per cycle,")
    add("// lowest index first == ascending (end, pattern_id).  `in_ready` is low")
    add("// while entries are pending, so the feed stalls rather than overrunning")
    add("// the ring.  Two hazards are handled explicitly below: (a) the in-flight")
    add("// beat is captured by a 1-deep skid (a dropped byte is a silent false")
    add("// negative), and (b) an overflowing entry still CLEARS its pend bit (or")
    ...
    if n == 1:
        ...
        add(f"    input  wire [{n-1}:0]  in_keep,")
    add("    input  wire        in_last,")
    add("    // engine backpressure (SR7): low while the result ring is draining.")
    add("    // A feeder MUST gate on it; the skid below covers the one beat it")
    ...
    add(f"    localparam [31:0] CIRC_ID3    = 32'h{circ_id[3]:08X};")
    add(f"    localparam [31:0] CIRC_FLAGS  = 32'h{circ_flags:08X};")
    add("    // NSLOTS is the SLOT count including tombstones (SR6): pattern_id is")
    add("    // an index into slots, never into the live subset, so every bound")
    ...
    add("    reg         running;")
    add("    reg         finishing;")
    add("    reg  [NPEND-1:0] pend;      // pending accepts: lane*%d + slot" % stride)
    add("    reg  [63:0] pend_base;      // byte_index of the beat that set `pend`")
    add("")
    add("    // --- HAZARD (a): 1-deep input skid (accept-and-hold) ---")
    add("    // in_ready is combinational from REGISTERED state, so a feeder reacts")
    add("    // one cycle late and exactly one beat can be in flight when it drops.")
    add("    // Capturing that beat here (instead of dropping it) is what keeps a")
    ...
    add("")
    add("    // fed_*: the beat actually presented to the datapath this cycle —")
    ...
    if n == 1:
        ...
        add(f"    wire [{8*n-1}:0] fed_data = skid_valid ? skid_data : in_data;")
    ...
    add("")
    add("    // ---- shared anchor/position wires (§6.5).  End-of-buffer anchors keep")
    add("    // the documented 1-byte-engine placeholder status (the model is")
    add("    // authoritative, R7).  Snort anchors are pure literals and lower to")
    add("    // automata with ZERO eps/assert edges, so on the SNORT-PF workload")
    add("    // these wires drive nothing at all. ----")
    for i in range(n):
        if i == n - 1:
            add(f"    wire lastk{i} = fed_keep[{i}];")
        else:
            add(f"    wire lastk{i} = fed_keep[{i}] && ~(|fed_keep[{n-1}:{i+1}]);")
    for i in range(n):
        add(f"    wire w{i} = {word_expr(f'b{i}')};")
    ...
    for i, au in enumerate(slots):
        add(f"    // ================= slot {i} (pattern_id {i}) =================")
        if au is None:
            add("    // TOMBSTONED (SR6): the index is retained and can never match,")
        ...
        add(f"    reg [NS{i}-1:0] st{i};        // registered one-hot state set")
        for j in range(n + 1):
        ...
        for j in range(n):
            ...
            add(f"        m{i}_{j}[START{i}] = 1'b1;  // continuous search seeding")
            subs = [("active[", f"a{i}_{j}["), ("moved[", f"m{i}_{j}["),
                    ("in_data", f"b{j}"), ("moved = moved;", f"m{i}_{j} = m{i}_{j};")]
            for ln in _rename(_emit_move_body(au), subs):
                add("        " + ln)
            if n > 1:
    ...
    add("    // (lane, slot) == ascending (end, pattern_id) — the ring order the")
    ...
    for k in range(npend - 1, -1, -1):
        ...
        if i < n_slots:
            ...
            terms.append("1'b0")             # slot-id padding to a power of two
    ...
    add(f"            if (pend[pe]) pend_idx = pe[{pw-1}:0];  // last write wins")
    add("    end")
    add(f"    wire [{slot_bits-1}:0] pend_slot = pend_idx[{slot_bits-1}:0];")
    if lane_bits > 0:
        add(f"    wire [{lane_bits-1}:0] pend_lane = pend_idx[{pw-1}:{slot_bits}];")
    add("")
    ...
    add("                endcase")
    add("            end")
    add("            // HAZARD (a): hold the beat the feeder could not retract.  The")
    add("            // guard (!skid_valid) and the consume branch's clear (guarded by")
    add("            // skid_valid) are mutually exclusive by construction, so the two")
    add("            // nonblocking writes below can never race for this register.")
    ...
    for i, au in enumerate(slots):
        if au is not None:
            add(f"                st{i} <= {{NS{i}{{1'b0}}}}; st{i}[START{i}] <= 1'b1;")
    add("            end else if (pend_any) begin")
    add("                // ---- drain: ONE result-ring entry per cycle ----")
    add("                // HAZARD (b): the pend bit is cleared UNCONDITIONALLY, out-")
    add("                // side the cap test.  An overflowing entry is dropped (OVF)")
    add("                // but must still retire, or pend never empties, in_ready")
    add("                // never rises, and both the feed and the finish drain hang.")
    add("                pend[pend_idx] <= 1'b0;")
    add("                if (out_count < out_cap) begin")
    add("                    res_wr <= 1'b1;")
    add("                    // res_start = 0: the harness emits on accept and never")
    add("                    // tracked the run's origin; the host recovers the true")
    add("                    // start from `end` + the slot's anchor length (SR7).")
    add("                    res_start <= 64'd0;")
    if lane_bits > 0:
    ...
    add(f"                    res_pattern_id <= {{{32-slot_bits}'d0, pend_slot}};")
    add("                    res_flags <= 32'd1;  // bit0 verified (advisory)")
    add("                    out_count <= out_count + 32'd1;")
    add("                end else begin")
    add("                    status <= status | 32'h0000_0008;  // OVF")
    add("                end")
    add("            end else if (finishing) begin")
    add("                // DONE only once the ring is complete: the drain branch has")
    add("                // higher priority, so every accept of the final beat is")
    add("                // already written.  Asserting DONE earlier would let the")
    add("                // wrapper compose the reply while entries were in flight.")
    add("                finishing <= 1'b0;")
    add("                running <= 1'b0;")
    add("                status <= (status & ~32'h0000_0001) | 32'h0000_0002;")
    add("                irq_status <= irq_status | 32'h0000_0001;")
    ...
    add("                pend      <= acc_flat;   // whole accept matrix of this beat")
    ...
    if n == 1:
        ...
        add("                byte_index <= byte_index + {56'd0, kept_n};")
    add("                if (skid_valid) skid_valid <= 1'b0;  // held beat retired")
...
def generate_group(patterns, flags_per_pattern=None,
    ...
    """Generate the SR7 pattern-SET circuit for ``patterns`` (N slots, 1 harness).
