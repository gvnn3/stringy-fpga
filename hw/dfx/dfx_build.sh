...
# OpenNIC static shell (pf=cmac=1) + one DFX reconfigurable partition
# `pyro_rp`
...
# dcp/open_nic_shell.bit         FULL flash image, SPIx4 -- feed to
# scripts/flash_u250.sh
# partials/id_stub.bit           ID stub as a PARTIAL (return to known-good
# baseline)
#
# The vendored shell under third_party/open-nic-shell is NEVER modified: the
# build
...
if {\$wns >= 0} { puts "OOC_ID_STUB_TIMING_MET" } \\
else { puts "OOC_ID_STUB_TIMING_FAILED" }
TCL
vivado -mode batch -nojournal -notrace -source "${OUT}/_ooc_id_stub.tcl" \
  -log "${OUT}/_ooc_id_stub.log" -journal /dev/null | tee \
    "${OUT}/_ooc_id_stub.out"
...
if grep -qi "Parameter BUILD16 bound to.*type: string" \
     "${OUT}/_ooc_id_stub.log"; then
  echo "ERROR: BUILD16 bound as a STRING, not an integer -- shell" >&2
  echo "  identity would be ASCII garbage." >&2
...
# 2025.2 synthesizes the top strictly and rejects OpenNIC's invalid
# $fatal("...")
...
[ -f "$XPR" ] || \
  { echo "ERROR: OpenNIC project not created; see ${OUT}/_proj.log"; exit 1; }
...
grep -q BUILD_STATIC_DONE "${OUT}/_static.out" || \
  { echo "build_static did not finish"; exit 1; }

# NOTE (inherited, expect this): the full OpenNIC shell does not fully close
# timing on 2025.2 -- residual setup violations live inside OpenNIC's own CMAC
# IP
# and the unused box_322mhz. Those are outside pyro_rp and do not block partial
# generation or pr_verify. What matters is that the pyro_rp region and
# box_250mhz
...
vivado -mode batch -nojournal -notrace -source "${DFX}/build_rm.tcl" \
  -tclargs "${DCP}/static_routed_locked.dcp" "${DCP}/pyro_id_stub.dcp" \
  "id_stub" "${OUT}" \
  -log "${OUT}/_rm_id_stub.log" -journal /dev/null | tee "${OUT}/_partials.out"

log "pr_verify"
vivado -mode batch -nojournal -notrace -source "${DFX}/pr_verify.tcl" \
  -tclargs "${OUT}" \
  -log "${OUT}/_prverify.log" -journal /dev/null | tee "${OUT}/_prverify.out"
grep -q PR_VERIFY_ALL_OK "${OUT}/_prverify.out" || \
  { echo "pr_verify reported failures"; exit 1; }
...
echo
echo "NEXT: flash the full image (see scripts/README.md -- note the live-PCIe"
echo "  hazard):"
echo "  PYRO_FLASH_ALLOW_LIVE_PCIE=1 scripts/flash_u250.sh flash"
echo "  ${DCP}/open_nic_shell.bit"
