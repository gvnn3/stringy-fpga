...
# open_run synth_1  -> mark pyro_rp HD.RECONFIGURABLE -> link the ID stub as
# the
# default child -> floorplan -> opt/place/route -> config0 -> black-box +
# lock.
#
# Outputs (the R82d artifact set):
# <out>/static_full_config0.dcp    routed static + ID-stub child (pr_verify
# reference)
# <out>/static_routed_locked.dcp   locked static; the substrate every partial
# is
# implemented against, bit-identical across configs (R82b)
# <out>/open_nic_shell.bit         FULL flash image (static + ID stub), SPIx4
# flash-boot
#
# Lessons inherited from the ebpf-os U250 DFX flow (integration/dfx) -- do not
# "simplify" these away:
# * `open_run synth_1` FROM THE PROJECT is required. It links the ~30 OOC IPs
# so
...
# * Re-query `get_cells` FRESH after every netlist edit. Cached cell handles
# go
...
if {[llength [rpcell]] != 1} {
  error "BUILD_STATIC: expected exactly 1 pyro_rp cell,\
 found [llength [rpcell]] -- pf=cmac=1?"
}
...
# 4. CDC: the QDMA-derived clock and the AXIS user clock are asynchronous
# (bridged
#    by async FIFOs). Group them so the manual DFX impl flow does not time those
# crossings as synchronous -- this is what the QDMA IP's own scoped
# constraints
...
puts "BUILD_STATIC: full flash bitstream ->\
 $out/open_nic_shell.bit (BUILD16=$build16)"
...
if {$wns >= 0} { puts "BUILD_STATIC_TIMING_MET" } \
else { puts "BUILD_STATIC_TIMING_FAILED" }
