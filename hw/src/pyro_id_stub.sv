// *************************************************************************
// PYRO Phase 2b - default reconfigurable-partition child: the ID stub (R80/R81).
//
// This is the `pyro_rp` child baked into the FULL static image at flash time.
// It is what lets `device_usable` flip true from a freshly flashed board, before
...
//   * ID_REQUEST    -> ID_REPLY, rp_child_id == 0 ("no pattern resident", R78.5a)
//                      + static_shell_id = (SPEC16 << 16) | BUILD16   (R81)
//   * MATCH_REQUEST -> STATUS/ERROR, code = PYRO_E_NOT_RESIDENT (7)   (R78.8/R80)
//   * PERF_REQUEST  -> STATUS/ERROR, code = PYRO_E_NOT_RESIDENT (7)   (no engine)
...
// this stub emits is <= 40 bytes, hence exactly ONE 64-byte beat, zero-padded to
...
module pyro_rp #(
  ...
  parameter [31:0] HARNESS_VERSION = 32'h0000_0000,// R78.5b wire/harness version
  ...
  wire         pyro_ok  = (h[8*14 +: 8] == PYRO_MAGIC) && (h[8*15 +: 8] == PYRO_VER);
      ...
      case (state)
        ...
        ST_BHDR: begin
          ...
          if (!rk_status) begin
            ...
            //   static_shell_id (4B BE) | harness_version (4B BE) | rp_child_id (4B BE)
            // static_shell_id = (SPEC16 << 16) | BUILD16  (R81)
